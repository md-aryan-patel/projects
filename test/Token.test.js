...
describe("Token contract: ", () => {
  ...
  describe("Minting token", () => {
    ...
    it("Should revert with error if the invested amount is less the 0.1 ether and Greater then 2 ether", async () => {
      ...
      expect(
        token.connect(account_1).mint({ value: investAmount })
      ).to.be.revertedWith("Investment amount is low");
      investAmount = toWei(2);
      expect(
        token.connect(account_1).mint({ value: investAmount })
      ).to.be.revertedWith("Investement amount is too high");
    });
  ...
  describe("Transfer Token ", () => {
    ...
    it("Should charge decided percenta of fees on not Exempted Accounts", async () => {
      tx = await token
        .connect(account_1)
        .transfer(account_2.address, transferAmount);
  ...
  describe("Burn tokens", () => {
    ...
    it("Should Transfer the ether according to the amount of token burned", async () => {
      ...
      balanceAfter =
        parseInt(fromWei(balanceAfter)) - parseInt(fromWei(balanceBefore));
